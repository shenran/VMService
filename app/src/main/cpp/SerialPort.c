 ...
 */
int set_Parity(struct termios *options, int databits, int stopbits, int parity) {
...
Java_android_serialport_api_SerialPort_open__Ljava_lang_String_2IIIII(JNIEnv *env, jclass type,
																	  jstring path_, jint baudrate,
																	  jint flags, jint databits,
	...
	{
		jboolean iscopy;
		const char *path_utf = (*env)->GetStringUTFChars(env, path_, &iscopy);
		LOGD("Opening serial port %s with flags 0x%x", path_utf, O_RDWR | flags);
		fd = open(path_utf, O_RDWR | flags);
		LOGD("open() fd = %d", fd);
		(*env)->ReleaseStringUTFChars(env, path_, path_utf);
		if (fd == -1)
		{
			/* Throw an exception */
...
JNIEXPORT jobject JNICALL
Java_android_serialport_api_SerialPort_open__Ljava_lang_String_2II(JNIEnv *env, jclass type,
																   jstring path_, jint baudrate,
																   jint flags) {
	const char *path = (*env)->GetStringUTFChars(env, path_, 0);
	...
	(*env)->ReleaseStringUTFChars(env, path_, path);
    return Java_android_serialport_api_SerialPort_open__Ljava_lang_String_2IIIII(env, type, path_, baudrate,flags,8,0,0);
}
